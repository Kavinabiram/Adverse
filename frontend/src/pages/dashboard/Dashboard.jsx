...
const Dashboard = () => {
    ...
    const [dates, setDates] = useState([new Date(2026, 3, 1), new Date(2026, 4, 31)]); // April 2026 to May 2026
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between space-y-4 md:space-y-0">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight dark:text-white">Overview</h2>
                    <p className="text-zinc-500 uppercase text-[10px] tracking-[0.2em] mt-1">Real-time platform performance metrics.</p>
                </div>
                <div className="flex items-center space-x-3">
                    ...
                    <button className="btn-primary font-bold">Download Report</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((stat, i) => (
                    <div key={i} className="card group dark:bg-black dark:border-zinc-800 transition-all duration-300 hover:border-black dark:hover:border-white">
                        ...
                        <p className="text-[9px] font-semibold text-zinc-500 uppercase tracking-[0.2em] mb-2">{stat.title}</p>
                        <h3 className="text-3xl font-bold text-black dark:text-white transition-colors duration-300">{stat.value}</h3>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 card dark:bg-black dark:border-zinc-800 transition-colors duration-300">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h4 className="text-lg font-semibold dark:text-white">Ads Performance</h4>
                            <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest mt-1">Monthly playback data.</p>
                        </div>
                        <div className="flex items-center space-x-2 text-[10px] font-semibold text-zinc-400 uppercase tracking-widest">
                        ...
                        {[1, 2, 3, 4, 5].map((_, i) => (
                            <div key={i} className="flex items-start space-x-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-zinc-500 mt-1">John Doe joined the platform.</p>
                                </div>
                                <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest bg-gray-50 dark:bg-zinc-900 px-2 py-1 rounded">2m ago</span>
                            </div>
                        ))}
                    </div>
                    <button className="w-full mt-8 py-3 text-xs font-bold uppercase tracking-widest border border-gray-100 dark:border-zinc-800 dark:text-zinc-400 hover:bg-gray-50 dark:hover:bg-zinc-900 rounded-lg transition-colors">
                        View All Logs
                    </button>
                </div>
            </div>
