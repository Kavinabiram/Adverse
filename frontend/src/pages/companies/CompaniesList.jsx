...
const CompaniesList = () => {
    ...
    const toggleStatus = async (rowData) => {
        try {
            const newStatus = rowData.status?.toLowerCase() === 'active' ? 'inactive' : 'active';
            await api.put(`/companies/${rowData.id}`, { ...rowData, status: newStatus });
            fetchCompanies();
        } catch (error) {
    ...
    const handleDelete = async (rowData) => {
        if (window.confirm('Are you sure you want to completely delete this account?')) {
            try {
                await api.delete(`/companies/${rowData.id}`);
                fetchCompanies();
            } catch (error) {
    ...
    const actionMenuItems = [
        {
            label: selectedCompany?.status?.toLowerCase() === 'active' ? 'Deactivate' : 'Activate',
            icon: selectedCompany?.status?.toLowerCase() === 'active' ? 'pi pi-ban' : 'pi pi-check-circle',
            command: () => toggleStatus(selectedCompany)
        },
        {
            separator: true
        },
        {
            label: 'Delete',
            icon: 'pi pi-trash',
            command: () => handleDelete(selectedCompany)
        }
    ];
    ...
    const actionsTemplate = (rowData) => (
        <div className="flex items-center justify-end space-x-2">
            ...
            <button
                ...
                className="p-2 hover:bg-gray-50 dark:hover:bg-zinc-900 rounded-lg text-gray-400 hover:text-black dark:hover:text-white transition-all"
            >
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="card !p-0 dark:bg-black dark:border-zinc-800 transition-colors duration-300 overflow-visible">
                <Menu model={actionMenuItems} popup ref={menuRef} id="popup_menu" className="text-xs p-1" appendTo={document.body} />
                <DataTable
                    value={companies}
                    lazy
                    paginator
